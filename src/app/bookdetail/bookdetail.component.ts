...
export class BookdetailComponent implements OnInit{
  ...
  loadBookDetail(){
    ...
    this.bookDetailService
      .getBookDetailWithPagination(this.currentPage, this.itemsPerPage)
      .pipe(
        tap((response) => {
          this.book_details = response.content;
          this.totalItems = response.totalItems;
          this.totalPages = response.totalPages;
          this.applyFilterBookDetail(); // Áp dụng bộ lọc sau khi tải dữ liệu
        }),
        catchError((error) => {
