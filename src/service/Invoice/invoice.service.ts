...
export class InvoiceService {
  private apiUrl = 'http://localhost:9000/paypal';
  ...
  createInvoice(invoice: any): Observable<any> {
    console.log('dữ liệu invoice data của console service:', invoice);  // In ra dữ liệu trước khi gửi
    return this.http.post<Invoice>('http://localhost:9000/invoice/create', invoice).pipe(
    );
  }
  payWithPayPal(invoice: any) {
    return this.http.post(`${this.apiUrl}/pay`, invoice, { responseType: 'text' });
  }
